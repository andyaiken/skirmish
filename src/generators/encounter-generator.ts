...
export class EncounterGenerator {
	static createEncounter = (region: RegionModel, heroes: CombatantModel[]): EncounterModel => {
		...
		const encounter: EncounterModel = {
			...
			combatants: [ ...heroes ],
			loot: [],
			mapSquares: region.pausedEncounter ? region.pausedEncounter.mapSquares : EncounterMapLogic.generateEncounterMap(rng)
		};

		const monsters: CombatantModel[] = [];
		if (region.pausedEncounter) {
			monsters.push(...region.pausedEncounter.combatants);
		} else {
		...
		if (region.pausedEncounter) {
			loot.push(...region.pausedEncounter.loot);
		} else {
